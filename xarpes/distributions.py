...
class fermi_dirac(unique_distribution):
    r"""Child class for Fermi-Dirac (FD) distributions, used e.g., during Fermi edge
    fitting. The FD class is unique, only one instance should be used
    per task.
    ...
    with :math:`A` as :attr:`integrated_weight`, :math:`B` as :attr:`background`,
    :math:`h\nu-\Phi` as :attr:`hnuminphi`, and :math:`\beta=1/(k_{\rm{B}}T)` with
    :math:`T` as :attr:`temperature`.
    ...
    def __init__(self, temperature, hnuminphi, background=0,
        ...
        @property
        def hnuminphi(self):
            r"""Returns the photon energy minus the work function of the FD distribution.
        ...
        def set_hnuminphi(self, x):
            r"""Sets the photon energy minus the work function of the FD distribution.
    ...
    def convolve(self, energy_range, energy_resolution):
        ...
        extend = np.linspace(energy_range[0] - enumb * step_size,
                             energy_range[-1] + enumb * step_size,
                             len(energy_range) + 2 * enumb)
        evalf = gaussian_filter(self.evaluate(extend), sigma=estep)[enumb:-enumb] 
        return evalf